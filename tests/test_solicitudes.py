...
# ── Override de la colección Mongo ────────────────────────────────────────────
# Los endpoints reciben la colección vía Depends(get_collection). FastAPI captura
# el objeto función en el registro de la ruta, así que parchear el namespace del
# router con patch() NO intercepta la dependencia. La forma correcta es
# app.dependency_overrides sobre la propia función get_collection.

@contextmanager
def _use_col(mock_col):
    app.dependency_overrides[get_collection] = lambda: mock_col
    try:
        yield
    finally:
        app.dependency_overrides.pop(get_collection, None)
